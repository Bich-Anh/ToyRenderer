...
std::string generate_filename(std::string name_prefix, std::string extension) {
    ...
    filename << name_prefix << "_"
             << std::put_time(&tm, "%Y-%m-%d_%H-%M-%S") // Format the time as YYYY-MM-DD_HH-MM-SS
             << "." << extension;

    return filename.str();
}

std::string format_elapsed_time(std::chrono::steady_clock::time_point start, std::chrono::steady_clock::time_point end) {
